...
use crate::schemas::auth_schema::{LoginRequest, RegisterRequest, ChangePasswordRequest, VerificationQuery, AuthMessageResponse};
...
pub fn protected_router() -> Router<AppState> {
    Router::<AppState>::new()
        ...
        .route("/newpass", patch(change_password))
}
...
pub async fn change_password(State(state): State<AppState>, Extension(claims): Extension<Claims>, Json(payload): Json<ChangePasswordRequest>) -> Result<Json<AuthMessageResponse>, AuthError> {
    auth_service::change_password(&state.db_pool, claims.sub, &payload.current_pass, &payload.new_pass).map(Json)
}
